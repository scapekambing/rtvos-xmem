...
class ResourceManager:
    def __init__(self, config):
        ...
        if need_decoding:
            self._extract_frames(self.workspace)
    ...
    def _extract_frames(self, video):
        ...
        files = [content for content in vid if os.path.isfile(os.path.join(video,content))]
        my_video = path.join(video, files[0])
        print('qqqqqqqqqq',vid,video,my_video)
        cap = cv2.VideoCapture(my_video)
        frame_index = 0
        print(f'Extracting frames from {my_video} into {self.image_dir}...')
